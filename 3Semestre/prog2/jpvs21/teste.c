...
void rotEsquerda(rbno_t *p){
    ...
    rbno_t* x=p->dir;
    x->p=p->p;
    if(p->p!=NULL){     //corrige o filho do avo
        if(p->p->esq==p)
            p->p->esq=x;
        else    
            p->p->dir=x;
...
char* busca_rb_interno(rb_t* arv, int chave){
...
char* busca_rb(rb_t* arv, int chave){
    if(arv->raiz!=NULL)
        return busca_rb_interno(arv, chave);
