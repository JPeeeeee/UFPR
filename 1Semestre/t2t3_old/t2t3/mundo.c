...
int main(){
    ...
    while(lef->Primeiro->evento->tipo != 4){
        ...
        switch(primeiro_evento->tipo){
            case 1:{
                chegada_t *chegada = malloc(sizeof(chegada_t));
                ...
                break;}
            case 2:{
                saida_t *saida = malloc(sizeof(saida_t));
                ...
                break;}
            case 3:{
                disseminacao_t *disseminacao = malloc(sizeof(disseminacao_t));
                disseminacao = primeiro_evento->dados;
		int NpessoasNoLocal = disseminacao->pessoasNoLocal->card;
		if (NpessoasNoLocal <= 0)
			NpessoasNoLocal = 1;
                conjunto_t *pessoasQueOuviram = cria_subconjunto(disseminacao->pessoasNoLocal, NpessoasNoLocal);
                int rumor;
                int j;
                ...
                for (j = 0; j < pessoasQueOuviram->card; j++){
                    ...
                    if(rand() % 100 < mundo->pessoas[pessoasQueOuviram->v[j]].extroversao){
                        ...
                        printf("(P%d:R%d)  ", disseminacao->idPessoa, rumor);
                    }
                }
                printf("\n");
                break;}
